...
(function () {
  ...
  function audioInit() {
    ...
    try {
      ...
      /* Bộ nén cuối đường: tiếng thắng cuộc có cả chục nốt chồng lên nhau,
         không nén thì đỉnh sóng vọt lên gấp mấy lần nhạc nền và chói tai. */
      let out = A.ctx.destination;
      try {
        const comp = A.ctx.createDynamicsCompressor();
        comp.threshold.value = -6; comp.knee.value = 16; comp.ratio.value = 4;
  ...
  const sfx = {
    ...
    sparkle(t0) {
      ...
      [1568, 2093, 2637, 3136].forEach((f, i) => tone(f, t + i * 0.045, 0.45, 'sine', 0.1));
    },

    /* ✅ đoán đúng chữ — ting kèm lấp lánh */
    good() {
      if (!A.ctx) return; const t = now();
      [659, 831, 988].forEach((f, i) => tone(f, t + i * 0.06, 0.24, 'triangle', 0.13));
      sfx.sparkle(t + 0.1);
    },
    ...
    victory() {
      if (!A.ctx) return; const t = now();
      [[523, 0], [659, .13], [784, .26], [1047, .39], [784, .56], [1047, .67], [1319, .8]]
        .forEach(function (x) { tone(x[0], t + x[1], 0.34, 'triangle', 0.17); tone(x[0] * 2, t + x[1], 0.24, 'sine', 0.072); });
      [131, 165, 196, 262].forEach((f, i) => tone(f, t + i * 0.13, 0.36, 'triangle', 0.2));
      sfx.sparkle(t + 0.86);
    },
    ...
    dramaticFail() {
      ...
      [392, 370, 349, 330].forEach((f, i) => {
        tone(f, t + i * 0.2, 0.42, 'sawtooth', 0.12);
        tone(f / 2, t + i * 0.2, 0.46, 'triangle', 0.14);
      });
      tone(62, t + 0.82, 1.0, 'sine', 0.24);
      noiseHit(t + 0.82, 0.7, 0.13, 80);
      sfx.whimper(t + 1.1);
    },
  ...
  function scheduleStep(s, t) {
    ...
    if (p % 2 === 0) {
      const m = MEL[bar][p / 2];
      if (m) tone(mtof(m), t, 0.28, 'triangle', 0.115, A.musicGain);
    }
    if (p === 0 || p === 6 || p === 8) tone(mtof(ch[0] - 24), t, 0.32, 'sine', 0.2, A.musicGain);
  ...
  function setMusic(on) {
    ...
    g.linearRampToValueAtTime(on ? 0.5 : 0.0001, t + 0.6);
